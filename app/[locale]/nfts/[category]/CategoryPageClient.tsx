...
function RarityChip({
  tier, label, percent, total, image, locale,
}: {
  tier: string; label?: string; percent?: number; total?: number; image?: string; locale: Locale;
}) {
  return (
    <span
      className={cx(
    ...
    >
      {image ? (
        <Image src={image} alt={label || tier} width={16} height={16} className="rounded-sm object-cover" />
      ) : (
